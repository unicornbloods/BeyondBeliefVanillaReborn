...
//#define CSKY
	...
	vec3 customFogColor = vec3(OFOGR, OFOGG, OFOGB) * 0.004;
	vec3 customSkyColor = vec3(CSKYR, CSKYG, CSKYB) * 0.004;
...
#if OFOG == 1
	vec3 fogcolor = fogColor * mix(fogColor, customFogColor, 1);
#else
	vec3 fogcolor = fogColor;
#endif

#ifdef CSKY
	vec3 skycolor = skyColor * mix(skyColor, customSkyColor, 1);
#else
