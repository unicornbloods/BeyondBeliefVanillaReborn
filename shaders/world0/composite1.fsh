...
#ifndef HEXBOKEH
 /* Sildur's default enhanced "fast blur" */
	float Bokeh[9] = float[9](-0.5, -0.375, -0.25, -0.125, 0.0, 0.125, 0.25, 0.375, 0.5);
#endif

#ifdef HEXBOKEH
	 /* Hex bokeh stolen from RRe36 who stole from choc, with permission */
	const vec2 Bokeh[60] = vec2[60] (
	    vec2(  0.2165,  0.1250 ),
		vec2(  0.0000,  0.2500 ),
		vec2( -0.2165,  0.1250 ),
		vec2( -0.2165, -0.1250 ),
...
#endif
	for ( int i = 0; i < 9; i++) {
		color += texture2D(texture, texcoord.xy + Bokeh[i]*pcoc*vec2(1.0,aspectRatio)).rgb;
	}
