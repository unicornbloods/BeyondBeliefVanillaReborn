...
#define waterCustom 1 //[0 1 2 3]
	...
	// Store all the water colors into a variable to reduce clutter
	vec4 WaterColor = vec4(waterRed, waterGreen, waterBlue, waterAlpha ) * 0.004;
...
void main() {
	...
	if (matID >= 0.1 && matID <= 1.5) {
		...
		#elif waterCustom == 2
			color = (texture2D(texture, texcoord) * WaterColor) * texture2D(lightmap, lmcoord);
		#elif waterCustom == 3 
			color = WaterColor * texture2D(lightmap, lmcoord);
		#endif
	}
